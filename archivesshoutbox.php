<?php
	...
	// On commence par vérifier le jour !
	if(!isset($_GET['jour']) OR $_GET['jour'] == '' OR $_GET['jour'] < '1' OR $_GET['jour'] > 31){
		header("location: errors.php?k=jour");
	}
	$jour = intval($_GET['jour']);
	
	// On fait la même chose avec le mois !
	if(!isset($_GET['mois']) OR $_GET['mois'] == '' OR $_GET['mois'] < '1' OR $_GET['mois'] > 12){
		header("location: errors.php?k=mois");
	}
	$mois = intval($_GET['mois']);
	
	// On fait la même chose avec l'année !
	if(!isset($_GET['annee']) OR $_GET['annee'] == '' OR $_GET['annee'] < '1970' OR $_GET['annee'] > $currentYear){
		header("location: errors.php?k=annee");
	}
	$annee = intval($_GET['annee']);
	
	$start = mktime(0, 0, 0, $mois, $jour, $annee);
	$stop = mktime(0, 0, 0, $mois, $jour + 1, $annee);
	
    $viewShouts = get_shouts($start, $stop, null);
    	...
    	<div id="tribune">
	    	<div id="optionbar">
	    		<div id="optionbarcontent">
	    			<?php
	    				display_calendar($mois, $annee);
	    			?>
	    		</div>
	    	</div>
    	<?php
        if($nbreMessages == 0){
        	echo "<div class='error'>Désolé il n'y a pas eu de message le ".$jour.' '.$arrayMois[$mois].' '.$annee." !</div>";
        }else{
        	?>
        	<H1 id="titretribune">LES MESSAGES DU <?php echo strtoupper($arrayJour[date('l', $start)]).' '.$jour.' '.strtoupper($arrayMois[$mois]).' '.$annee; ?></H1>		
			<div id="poststribune">
			<?php
