<?php
    $title = 'Recherche dans la tribune';
    require '_header.php';

	// On commence par vérifier la requete
	if(!isset($_POST['what']) OR $_POST['what'] == ''){
		echo '<pre>';
		print_r($_POST);
		echo '</pre>';
		exit();
		header("location: errors.php?k=what");
	}
	$what = mysql_real_escape_string($_POST['what']);

    // On commence par récupérer les derniers shouts pour les afficher
    $viewShouts = get_shouts(null, null, $what);
...
<div id="main"><!-- MAIN -->
    <div class="content"><!-- CONTENT -->
        ...
        <?php
       	if($nbreMessages == 0){
       		echo "<div class='error'>Désolé il n'y a pas de correspondance avec votre recherche ( <i>".$what."</i> ) !</div>";
       	}else{
       		?>
       		<H1 id="titretribune">VOTRE RECHERCHE : " <i><?php echo strtoupper($what); ?></i> "</H1>		
			<div id="poststribune">
       		<?php
